...
async function sendAdminThreadReply(
  ...
  attachments: string[] = [], 
  identity?: AdminIdentity
): Promise<MessageAPI | { error: string; visitorEmail?: string; visitorName?: string }> {
                    ...
                    <div className="flex items-center mb-1">
                      <div className="w-8 h-8 bg-gradient-to-br from-green-500 to-blue-600 rounded-full flex items-center justify-center text-white text-xs font-semibold mr-3 flex-shrink-0">
                        {threads.find(t => t?.threadId === conversation?.conversationId)?.isVisitor 
                          ? "V" 
                          : (conversation?.withUser?.name || "U").split(' ').map(n => n[0]).join('').toUpperCase()
                        }
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center">
                          <span className="font-medium text-gray-900 text-sm md:text-base truncate">
                            {conversation?.withUser?.name || "Utilisateur inconnu"}
                          </span>
                          {threads.find(t => t?.threadId === conversation?.conversationId)?.isVisitor && (
                        ...
                        <div className="text-xs text-green-600 font-medium">
                          {threads.find(t => t?.threadId === conversation?.conversationId)?.isVisitor ? "Visiteur" : "Client"}
                        </div>
                      </div>
                    </div>
...
<<<<<<< HEAD
                ...
                <div>
                  ...
                  <div className="mt-1 text-xs text-gray-500">
                    Cette identité sera visible par le client dans la conversation
                  </div>
                </div>
