...
>
> **Dernière mise à jour** : **15 Juillet 2025** – couverture tests : **63 / 63 passed – 87 %**.
...
- **tests/** : Architecture de tests séparés (unitaires CI/CD + intégration local) + Cypress E2E
- **vite.config.ts** : Configuration CI/CD (tests unitaires uniquement)
- **vite.config.integration.ts** : Configuration complète (tous tests pour local)
...
4. **Tests** :
   ...
   - **Architecture séparée** : unitaires (CI/CD) vs intégration (local)
   - **CI/CD optimisé** : tests unitaires uniquement, pas de dépendances backend
   - **Tests d'intégration** : exclus du CI, nécessitent backend en fonctionnement
   - **Tests conditionnels S3** : via `skipIfNoAws` pour éviter échecs CI
...
- **Architecture CI/CD robuste** :
  - GitHub Actions : tests unitaires uniquement (`test:unit`)
  - Tests d'intégration : exclus du CI, exécutés en local
  - Configuration séparée : `vite.config.ts` (CI) vs `vite.config.integration.ts` (local)
- Pipeline GitHub Actions lance :
  - Lint → Tests backend → Tests frontend (unitaires) → Build Docker multi‑arch.
...
| Tests frontend échouent en CI       | Utiliser `test:unit` pour CI, `test:integration` pour local avec backend.     |
